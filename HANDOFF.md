...
0.4.0 已由 Actions 34761341850 发布两平台 ZIP，提交 945467a。0.4.1 使用统一 i18n 模板补动态标签，移除整个灵动岛 translate=no 和无用属性。JS/core、macOS 构建和签名通过。0.4.1 发布状态见后续记录。

原生 `.ui-artifacts/1789309445622-hover/` 的 10 次悬停循环、三环和缓冲条通过，但“点击打开详情”失败；脚本已改成展开完成后取当前 AX 控件坐标并前置测试窗口，复验 `.ui-artifacts/1789309667756-hover/` 因锁屏 UNTESTED，仍需验收。以前 69 项通过属于 0.4.0 开发中版本，不能作为当前最终构建全部通过证据。系统通知横幅与权限仍未实机验证。完整 goal 保持 active；Windows 实机、GUI Agent 请求和多批真实基准仍待完成。
...
# 2026-09-13 — v0.4.1 原生回归完成

- 原生完整回归 `.ui-artifacts/1789311386669-ui/result.json`：105 项 PASS。覆盖待命 Agent 展开、点击核验环打开正确四元组与核验 Tab、概览三环、Cache/TTFT/核验跨窗口跳转、导出 JSONL、引导、拖动吸附和菜单退出。
- 原生悬停专测曾因锁屏或指针坐标偏移为 UNTESTED；完整 `ui` 回归在桌面可操作后已通过。验收脚本现在按实际 `main-geometry.json` 的 selectedId/view 验证跨窗口导航，并在全待命时先通过“返回全部 Agent”展开模型。
