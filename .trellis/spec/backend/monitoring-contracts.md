...
- A macOS app launched from Finder does not inherit terminal-only provider variables. For a live Agent owned by the same user, read only known model, endpoint, and credential variable names from that process and use them to resolve its existing provider configuration; never expose or log the process environment. Absence of an active-probe credential is a passive-monitoring state, not an integration failure.
...
- Passive Codex quota uses `rate_limits` windows from the local rollout. Classify windows by `window_minutes`, display `100 - used_percent` as remaining allowance, preserve the upstream reset timestamp and observation time, and keep quota separate from monetary balance. Quota belongs to the routed account and may be reused by idle Codex sessions only when their `model_provider` matches; per-turn Cache and TTFT must never be copied this way. Stop displaying a window after its recorded reset time; do not infer that the refreshed allowance is 100%. A custom relay such as CLIProxyAPI represents the most recently routed upstream credential, not a pool-wide balance. Missing quota or credit values remain unavailable, never zero.
- UI model-to-probe matching must tolerate catalog normalization arriving before or after probe discovery. Match the route tuple and observed model when only one side has a canonical model ID; do not show an integration error during this synchronization window.
- Codex writer-lock sessions resolve `model_provider` from the thread through `agentConnections(..., { codexProvider })`. A different global provider cannot lend that thread its shared `auth.json` key or `OPENAI_BASE_URL`; only the selected provider's explicit credentials remain eligible. Read generic process rows before thread routes so a resume process cannot overwrite thread-specific routing. Keep the provider consistency check before probing.
- `probeEnabled` is the global automatic-verification/sampling switch, not a per-model switch. A manual run does not change it. `MODIVUE_UI_ARTIFACTS` pauses scheduling and omits live balance providers; expose the pause reason, and never deliver that process as the normal daily application. Changing `verificationIntervalMinutes` must reschedule the next check.
