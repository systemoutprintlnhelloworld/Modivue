...
- Passive Codex quota uses `rate_limits` windows from the local rollout. Classify windows by `window_minutes`, display `100 - used_percent` as remaining allowance, preserve the upstream reset timestamp and observation time, and keep quota separate from monetary balance. Stop displaying a window after its recorded reset time; do not infer that the refreshed allowance is 100%. A custom relay such as CLIProxyAPI represents the most recently routed upstream credential, not a pool-wide balance. Missing quota or credit values remain unavailable, never zero.
