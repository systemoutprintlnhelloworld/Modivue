...
export async function configureBalance(input) {
  ...
  if (provider.framework === "cliproxyapi") throw new TypeError("CLIProxyAPI 余量查询尚未接入管理 API");
  ...
  inFlight = (async () => {
    ...
    const rows = await Promise.all((await providers()).map(async provider => {
      if (provider.framework === "cliproxyapi") return {
        ...
        balanceSupported: false, status: "unsupported", message: "CLIProxyAPI 余量查询尚未接入管理 API", history: []
      };
