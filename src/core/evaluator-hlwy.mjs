...
async function run(input) {
  ...
  const attempts = input.sampleCount ?? 50;
  if (!Number.isInteger(attempts) || attempts < 50 || attempts > 500) throw new TypeError("HLWY 样本数必须为 50–500 的整数");
  ...
  const canResume = previousRun?.status === "paused" && previousRun.evaluator_version === hlwyVersion
    ...
    && previous.wireApi === input.wireApi && previous.reasoningEffort === (input.reasoningEffort || null)
  ...
  const metadata = { verdict: "inconclusive", source: reference.source, revision: reference.timestamp,
    ...
    conditionNotice: reference.trusted ? "使用同协议、档位、提示词、温度和输出上限的可信 API 分布；相似度不等于身份认证。"
  ...
  if (numbers.length < 40 || numbers.length / attempts < .8) return { status: "unsupported",
    rationale: `有效样本不足：${numbers.length}/${attempts}，至少 40 个且有效率不低于 80%`, metadata };
  ...
  return { status: "ok", rationale: `HLWY 匹配度 ${result.value.toFixed(2)}% · ${numbers.length} 个有效样本`,
    metadata: { ...metadata, ...result, directedModel: closest?.model || null,
      directionScore: closest?.relativeMatch ?? null, candidateDistribution } };
}
