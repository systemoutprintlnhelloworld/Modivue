...
| web | 系统 Chrome 验证数值、自动保存、核验折叠/历史/导出、悬停指标、44 个英文场景及中英往返切换；不调用付费模型 |
...
英文检查覆盖主视图、所有方法、设置分类、展开报告、灵动岛三种形态及中英往返切换，扫描可见文字和 title/placeholder/aria-label。任何遗漏使任务 FAIL；查看运行目录内 `i18n-coverage.json`。题目原文、模型回答、原始 JSON 与用户自定义文本保持源语言。
