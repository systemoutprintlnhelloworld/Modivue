...
assert.equal(isAgentWorking({status:'running',source:'process',metadata:{pid:1}}), true);
assert.equal(isAgentWorking({host:'pi',status:'running',source:'process',statusSource:'process-presence',metadata:{pid:1}}), true);
assert.equal(isAgentWorking({host:'codex',status:'running',source:'process',metadata:{pid:1}}), false);
assert.equal(isAgentWorking({host:'claude-code',status:'running',source:'process',metadata:{pid:1}}), false);
